...
class MultiGraphEnsembleFC(torch.nn.Module):
    ...
    def forward(self, x, sl_pos, sl_neg, kg_ppi, kg_reactome, kg_corum, kg_go_f, kg_go_c, kg_go_p, kg_kegg):
        ...
        #features_sum = torch.sum(features_all, dim=-1)
        #features_all =  torch.cat((features_all, features_sum.unsqueeze(1)), axis=1)
...
class MultiGraphEnsembleCNN(torch.nn.Module):
    ...
    def forward(self, x, sl_pos, sl_neg, kg_ppi, kg_reactome, kg_corum, kg_go_f, kg_go_c, kg_go_p, kg_kegg):
        ...
        #node embedding 
        emb_all = [self.emb_sl, self.emb_reactome, self.emb_sl, self.emb_kegg, self.emb_sl, self.emb_go_f, self.emb_sl, self.emb_corum, self.emb_sl, self.emb_go_c, self.emb_sl, self.emb_go_p, self.emb_sl, self.emb_ppi]
